...
function cargarProductoRopa() {
  ...
  // producto
  const producto = obtenerProducto('ropa', idProducto);
  if(!producto) {
...
function renderBreadcrumb(producto) {
  ...
  const breadcrumbHtml = `
    ...
    <li class="breadcrumb-item"><a href="${window.BASE_URL}pages/ropa.html">Ropa</a></li>
...
function renderCompra(producto) {
  ...
  const tallasHtml = Object.entries(producto.stock)
    .map(([talla, cantidad]) => 
      `<option value="${talla}">${talla} (${cantidad} disponibles)</option>`
    )
    .join('');
  const compraHtml = `
    <h3>Comprar</h3>
